...
/*Put the initial conditions again for the linear run*/
for(i=0;i<Nr+1;i++){
	delta[i] = d0*delta0[i]/delta0[0];
	vel[i] = exp(nini)*delta[i];
}
...
while(n < nc[0] && nc[0] < nmax){
	...
	/*Evolve all shells one step in time*/
	Evolve_Linear(n, r, k, delta, vel, h, nr, nk);
	n = n + h;
...
fprintf(outp, "%lf %lf %lf", exp(nc[0]), exp(nc[0])*errnc, dc);
